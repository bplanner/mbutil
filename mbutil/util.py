...
def mbtiles_to_disk(mbtiles_file, directory_path, **kwargs):
    ...
    logger.info('Exporting %s tiles...' % (count))
    t = tiles.fetchone()
    while t:
        z = t[0]
        x = t[1]
    ...
    logger.info('%s tiles exported' % (count))
    ...
    logger.info('Exporting %s grids...' % (count))
    count = 0
    while g:
        zoom_level = g[0] # z
        tile_column = g[1] # x
    ...
    logger.info('%s grids exported' % (count))
